/* Example file to create a cgroup object,
   Run following commands:
cgcreate -g cpu,cpuset,cpuacct,net_cls:/dadvisor
cgexec -g cpu,cpuset,cpuacct,net_cls:/dadvisor stress -c 2 &
cgexec -g cpu,cpuset,cpuacct,net_cls:/dadvisor terminator
./bin/cgroup_test dadvisor/
killall stress
cgdelete -g cpu,cpuset,cpuacct,net_cls:/dadvisor
 */
...
int main(int argc, char *argv[]) {
  if(argc != 2) {
    cout<<"usage: "<<argv[0]<<" <cgroup>"<<endl;
  } else {
    CGroup cgroup("test-cgroup", argv[1], "lo");
    cgroup.setHardCPUShares(50);
    cgroup.setNetworkOutBW(500*1024);
    sleep(2);

    SimpleMonitor monitor(&cgroup, true, true);
    monitor.run();
  }
